...
public interface CommandRepo extends JpaRepository<Command, Long> {
    ...
    @Query(value="Select * from Command where command=?1 and className=?2",nativeQuery = true)
    Command findRouteByClass(String route,String className);

    @Query(value="Select * from Command where commandType=?1 and command=?2 and systemCommand=?3 ",nativeQuery = true)
    Command findCommand(String commandType,String command,boolean system);
    
    @Query(value="Select * from Command where systemCommand=?1",nativeQuery = true)
