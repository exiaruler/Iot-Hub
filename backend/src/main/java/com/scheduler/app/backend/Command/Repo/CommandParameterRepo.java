...
public interface CommandParameterRepo extends JpaRepository<CommandParameter,Long> {
    ...
    @Query(value="select * from command_parameter where command_id=?1 and pin=true order by parameter_order",nativeQuery = true)
    List<CommandParameter> findPinParameters(long id);

    @Query(value="select * from command_parameter where command_id=?1 ",nativeQuery = true)
    List<CommandParameter> findParametersByCommand(long id);

    @Query(value="select * from command_parameter where id=?1",nativeQuery = true)
